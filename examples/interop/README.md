...
| [`browser/`](./browser/) | the DOM, via Pyodide's `js` | in a browser | `pyfun bundle`: the program as a static page; a typed `Dom` façade module (`extern import js`, opaque `Element`/`Event`, `create_proxy` hidden behind `Dom.proxy`); a click handler is an ordinary one-parameter function; top-level `let mut` state |
